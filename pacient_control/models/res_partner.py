...
class Resident(models.Model):
    ...
    def action_see_basic_calendar(self):
        ...
        return {
            ...
            "view_id": self.env.ref(
                "pacient_control.project_task_pacient_view_filtered"
            ).id,
            "domain": [
                "&",
                ("partner_id", "=", self.id),
                ("tasktype_id", "=", self.env.ref("pacient_control.BC_type").id),
            ],
            "context": {
                "default_project_id": self.sudo().residence_id.project_ids[0].id,
                "default_partner_id": self.id,
                "default_tasktype_id": self.env.ref("pacient_control.BC_type").id,
            },
        }
    ...
    def action_see_treatment(self):
        ...
        return {
            ...
            "domain": [
                "&",
                ("partner_id", "=", self.id),
                ("tasktype_id", "=", self.env.ref("pacient_control.T_type").id),
            ],
            "view_id": self.env.ref(
                "pacient_control.project_task_pacient_view_filtered"
            ).id,
    ...
    def action_see_analysis_calendar(self):
        ...
        return {
            ...
            "domain": [
                "&",
                ("partner_id", "=", self.id),
                ("tasktype_id", "=", self.env.ref("pacient_control.A_type").id),
            ],
            "context": {
                "default_project_id": self.sudo().residence_id.project_ids[0].id,
                "default_partner_id": self.id,
                "default_tasktype_id": self.env.ref("pacient_control.A_type").id,
            },
        }
    ...
    def action_see_hygiene_calendar(self):
        ...
        return {
            ...
            "domain": [
                "&",
                ("partner_id", "=", self.id),
                ("tasktype_id", "=", self.env.ref("pacient_control.H_type").id),
            ],
            "context": {
                "default_project_id": self.sudo().residence_id.project_ids[0].id,
                "default_partner_id": self.id,
                "default_tasktype_id": self.env.ref("pacient_control.H_type").id,
            },
        }
    ...
    def action_see_tasks_calendar(self):
        ...
        return {
            ...
            "view_mode": "calendar",
            "res_model": "project.task",
            "view_id": self.env.ref("pacient_control.project_task_pacient_view").id,
    ...
    def action_see_tasks_monitoring(self):
        ...
        return {
            ...
            "domain": [
                "&",
                ("partner_id", "=", self.id),
                ("tasktype_id", "=", self.env.ref("pacient_control.M_type").id),
            ],
            "context": {
                "default_project_id": self.sudo().residence_id.project_ids[0].id,
                "default_partner_id": self.id,
                "default_tasktype_id": self.env.ref("pacient_control.M_type").id,
            },
        }
