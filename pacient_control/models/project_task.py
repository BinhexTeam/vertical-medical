...
class ProjectTask(models.Model):
            ...
            .search(
                [
                    (
                        ...
                        "=",
                        self.env.ref(
                            "residence_management.documents_residents_documents_analitica"
                        ).id,
                    )
                ]
            )
    ...
    def hygiene_events(self):
        ...
        if self.tasktype_id == self.env.ref("pacient_control.H_type"):
            elem_1 = self.env["rm.resident.control.hygiene.element"].create(
                {"name_id": self.env.ref("pacient_control.hygiene_elem_shower").id}
            )
            elem_2 = self.env["rm.resident.control.hygiene.element"].create(
                {"name_id": self.env.ref("pacient_control.hygiene_elem_shave").id}
            )
            elem_3 = self.env["rm.resident.control.hygiene.element"].create(
                {"name_id": self.env.ref("pacient_control.hygiene_elem_teeth").id}
            )
